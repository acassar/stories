...
| Un message deja envoye affiche la valeur d'aujourd'hui | Le fil interpole tout le passe avec les variables courantes, faute de garder un instantane par etape. Remonter la conversation peut donc montrer « 1 nuits » la ou « 3 nuits » avait ete envoye. Le corriger demanderait d'alourdir `GameState` d'une copie des variables a chaque pas — cher, pour un cas qui ne se voit qu'en relisant, et surtout sur les compteurs. |
