...
| FMT-5 | Interpolation de variables dans le texte  | ✅       | M      | `interpolate` dans `story-format`, appliquée par le moteur : le lecteur et le playtest du studio l'ont sans rien changer. Substitution de noms déclarés, jamais d'exécution. |
...
| FMT-7 | Attentes en temps réel                    | ✅       | M      | `Scene.waitMinutes` : le correspondant se tait pour de vrai, douze heures veulent dire douze heures. L'état note **quand** le silence a commencé, jamais quand il finit — l'allure choisie par le lecteur est appliquée à l'affichage par `waitStatus`, donc la déplacer en plein silence le recalcule sans rien reprogrammer. Sur le nœud et pas sur le lien : une nuit dure une nuit quel que soit le chemin. |
| RD-12 | Allure des attentes, côté lecteur         | ✅       | S      | Du temps réel à « sans attente », dans une feuille de réglages atteignable depuis le rail et depuis la conversation. Jamais dans la sauvegarde : une partie ouverte ailleurs se joue à l'allure de qui la lit. `allowedPaces` est la seule ligne à changer le jour où le haut de l'échelle se paie. |
| RD-13 | Prévenir quand le correspondant revient   | P1       | L      | Rien sur le web ne réveille un navigateur endormi : il faut du Web Push, donc un service qui tienne le réveil, et sur iOS une PWA installée. **Décision de plateforme, pas de fonctionnalité** — le socle FMT-7 n'en dépend pas, il se branche là où `awaitingSince` est posé. À chiffrer avec RD-11 (service worker) et PLAT-1 (Tauri, qui rendrait tout ça local). |
